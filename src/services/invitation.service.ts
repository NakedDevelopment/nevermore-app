...
import { APPWRITE_DATABASE_ID, APPWRITE_INVITATIONS_COLLECTION_ID, APPWRITE_INVITATION_FUNCTION_ID } from '@env';
...
private async sendInvitationEmail({ email, deepLink }: { email: string; deepLink: string }): Promise<void> {
  if (!APPWRITE_INVITATION_FUNCTION_ID) {
    throw new Error(
  ...
  const execution = await functions.createExecution({
    functionId: APPWRITE_INVITATION_FUNCTION_ID,
    body: JSON.stringify({ email, deepLink }),
  });

  let parsedBody: { success?: boolean; message?: string } = {};
  try {
    parsedBody = execution.responseBody ? JSON.parse(execution.responseBody) : {};
  } catch {
  }

  const failedStatus = typeof execution.responseStatusCode === 'number' && execution.responseStatusCode >= 400;
  if (failedStatus || parsedBody.success === false) {
    throw new Error(parsedBody.message || 'Failed to send invitation email.');
  }
}
...
async createInvitation({
  ...
  try {
  ...
  } catch (invitationEmailError: any) {
    try {
    ...
    throw new Error(`Failed to create invitation: ${invitationEmailError?.message || 'Unknown error'}`);
  }
...
async resendInvitation(invitation: Invitation): Promise<Invitation> {
  ...
  try {
    await this.sendInvitationEmail({ email: invitation.email, deepLink });
  } catch (invitationEmailError: any) {
    throw new Error(`Failed to resend invitation: ${invitationEmailError?.message || 'Unknown error'}`);
  }
